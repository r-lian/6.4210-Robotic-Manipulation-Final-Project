"""
ICP with optimized camera positions for better brick1 coverage.
Key insight: Brick1 at [-0.40, -0.15] is poorly covered by current cameras.
"""
...
np.random.seed(42)
brick_positions = [
    [-0.40, -0.15, np.random.uniform(0, np.pi)],  # Problematic brick
...
# Create SDFs
table_path = Path("/tmp/table_cov.sdf")
...
brick_path = Path("/tmp/brick_cov.sdf")
sx, sy, sz = brick_size
...
# IMPROVED: Better camera positioning for brick1 coverage
# Brick workspace: X∈[-0.5, -0.2], Y∈[-0.2, 0.2]
# Camera strategy: surround the workspace more evenly
scenario_yaml = f"""
directives:
- add_model:
    name: iiwa
...
{brick_directives}
# Camera 0: Front-right (good for brick3 at -0.25, 0)
- add_frame:
    name: camera0_origin
    X_PF:
        base_frame: world
...
# Camera 1: Left side (good for brick1 at -0.40, -0.15 and brick2)
- add_frame:
    name: camera1_origin
    X_PF:
        base_frame: world
        rotation: !Rpy {{ deg: [-120.0, 0.0, 60.0]}}
        translation: [-0.65, -0.45, 0.5]
- add_model:
    name: camera1
    file: package://manipulation/camera_box.sdf
- add_weld:
...
# Camera 2: Right side (good for brick2 at -0.35, 0.15)
- add_frame:
    name: camera2_origin
    X_PF:
        base_frame: world
        rotation: !Rpy {{ deg: [-120.0, 0.0, -100.0]}}
        translation: [-0.1, -0.75, 0.5]
- add_model:
    name: camera2
    file: package://manipulation/camera_box.sdf
- add_weld:
...
# Camera 3: Back-center (specifically for brick1 at -0.40, -0.15)
- add_frame:
    name: camera3_origin
    X_PF:
        base_frame: world
        rotation: !Rpy {{ deg: [-110.0, 0.0, 35.0]}}
        translation: [-0.7, 0.2, 0.55]
- add_model:
    name: camera3
    file: package://manipulation/camera_box.sdf
- add_weld:
...
print("=== ICP WITH OPTIMIZED CAMERA COVERAGE ===")
print("\nCamera positioning:")
print("  Camera 0: Front-right [-0.1, 0.8, 0.5] → covers brick3")
print("  Camera 1: Left-back [-0.65, -0.45, 0.5] → SPECIFICALLY for brick1")
print("  Camera 2: Right-back [-0.1, -0.75, 0.5] → covers brick2")
print("  Camera 3: Back-left [-0.7, 0.2, 0.55] → EXTRA coverage for brick1")
print("\nGoal: Increase brick1 point count from 5,968 to >7,000\n")
...
print("="*70)
print("ICP RESULTS (OPTIMIZED COVERAGE)")
print("="*70)
...
for i in range(1, 4):
    ...
    # Larger crop for better coverage
    brick_lower = X_PC_brick.translation() + np.array([-0.25, -0.25, -0.25])
    brick_upper = X_PC_brick.translation() + np.array([0.25, 0.25, 0.25])
    ...
    # Less aggressive downsampling: 4mm instead of 5mm
    downsampled = no_table.VoxelizedDownSample(0.004)

    print(f"  Points after processing: {downsampled.size()}")
...
print(f"Graspable bricks (<50mm): {graspable}/3")
if graspable == 3:
    print("✓✓✓ SUCCESS: ALL 3 BRICKS GRASPABLE!")
else:
    print(f"⚠ FAILED: Only {graspable}/3 bricks graspable")
print(f"{'='*70}")
