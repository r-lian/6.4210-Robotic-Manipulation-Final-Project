...
"""
    ...
    # RRT for collision-free joint-space path planning with optional moving obstacles
    def rrt_plan(
        q_start: np.ndarray,
        q_goal: np.ndarray,
        iiwa_name: str,
        ...
        """
        Simple RRT planner in joint space with collision checking.
        If other_arm_trajectory is provided, checks collision against moving obstacles.
        ...
        # If no trajectory provided, use current position
        if other_arm_trajectory is None:
            sim_ctx = simulator.get_mutable_context()
            sim_plant_ctx = plant.GetMyContextFromRoot(sim_ctx)
            q_other_static = plant.GetPositions(sim_plant_ctx, other_iiwa_model)
            other_arm_trajectory = [q_other_static]

        # Collision checker with moving obstacles
        def is_collision_free(q, time_fraction=0.5):
            plant.SetPositions(plant_ctx, iiwa_model, q)
            # Interpolate other arm position based on time
            idx = int(time_fraction * (len(other_arm_trajectory) - 1))
            idx = max(0, min(idx, len(other_arm_trajectory) - 1))
            plant.SetPositions(plant_ctx, other_iiwa_model, other_arm_trajectory[idx])
        ...
        # RRT tree: list of (config, parent_index)
        tree = [(q_start.copy(), -1)]
        ...
        for iteration in range(max_iterations):
            ...
            # Time fraction through the path (estimate)
            time_fraction = float(len(tree)) / max(max_iterations, 1)

            # Collision check with interpolated other arm position
            if not is_collision_free(q_new, time_fraction):
                continue
    ...
    def plan_arm_task(arm_name: str, task: ArmTask):
        ...
        # Reserve obstacle space (discretize RRT path into time steps)
        for i, q in enumerate(rrt_path):
            if i not in obstacle_reservations:
                obstacle_reservations[i] = {}
            obstacle_reservations[i][arm_name] = q

        print(f"[{arm_name}] Planned brick {task.brick_idx}: RRT {len(rrt_path)} waypoints, reserved timesteps 0-{len(rrt_path)-1}")
    ...
    def setup_arm_motion(arm_name: str, plan: dict):
        ...
        This ONLY sets inputs - does NOT advance simulator.
        Returns list of (phase, end_time) tuples.
        """
        iiwa_name = f"iiwa_{arm_name}"
        wsg_name = f"wsg_{arm_name}"
        source = iiwa_left_trajectory_source if arm_name == "left" else iiwa_right_trajectory_source
        ...
        print(f"[{arm_name}] Phase 1: Moving to prepick (duration={duration:.2f}s)")

        return [(ExecutionPhase.MOVING_TO_PREPICK, t_now + duration)]
    ...
    def process_arm_execution(arm_name: str):
        """
        Process execution phase for an arm.
        Returns True if execution work was done, False if not ready to execute
        """
        global arm_states, arm_plans, obstacle_reservations
        ...
        if arm_states[arm_name] == ArmState.EXECUTING:
            ...
            execute_full_pick_place(arm_name, plan)

            # Clear reservations for this arm after execution
            ...
            arm_states[arm_name] = ArmState.IDLE
            print(f"[{arm_name}] Execution complete, returning to IDLE")
            return True
    ...
    def run_dual_arm_system(tasks: list[tuple]):
        """
        Main coordinator: assigns tasks to queues and runs arms concurrently.
        ...
        # Keep processing until both queues empty and arms idle
        while not left_task_queue.empty() or not right_task_queue.empty() or \
            ...
            # PHASE 1: Let both arms plan concurrently (if idle)
            process_arm_planning("left")
            process_arm_planning("right")

            # PHASE 2: Execute one arm that's ready (priority: whoever finished planning first)
            # Only one can execute at a time due to simulator constraints
            executed = process_arm_execution("left") or process_arm_execution("right")

            # If both arms idle and queues empty, we're done
            if left_task_queue.empty() and right_task_queue.empty() and \
               arm_states["left"] == ArmState.IDLE and arm_states["right"] == ArmState.IDLE:
                break

            # Small yield only if nothing happened
            if not executed:
                time.sleep(0.001)
