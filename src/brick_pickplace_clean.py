...
- add_model:
    ...
    default_free_body_pose:
        brick_link:
            translation: [-0.35, 0, {brick_size[2]/2.0 + 0.001}]
...
def design_grasp_pose(X_WO: RigidTransform) -> tuple[RigidTransform, RigidTransform]:
    """Design grasp pose - centered on brick for proper grasping."""
...
def design_pregoal_pose(X_WG: RigidTransform) -> RigidTransform:
    """Pre-goal hover - lift brick straight up in world frame before moving to goal."""
    # Lift in world frame (not gripper frame) to go straight up
    current_pos = X_WG.translation()
    lifted_pos = current_pos + np.array([0.0, 0.0, 0.2])  # Lift 20cm in world Z
    return RigidTransform(X_WG.rotation(), lifted_pos)


def design_goal_poses(X_WO: RigidTransform, X_OG: RigidTransform) -> RigidTransform:
    """Goal pose - place brick symmetrically on the other side (mirror across Y-axis)."""
    # Get current brick position
    brick_pos = X_WO.translation()
    # Mirror horizontally across Y-axis: if brick is at [x, 0, z], place at [-x - offset, 0, z]
    # This moves it to the opposite side of the table along X direction
    goal_pos = np.array([-brick_pos[0] - 0.35, brick_pos[1], brick_pos[2]])
    X_WOgoal = RigidTransform(X_WO.rotation(), goal_pos)
    X_WGgoal = X_WOgoal @ X_OG
    return X_WGgoal


def design_postgoal_pose(X_WG: RigidTransform) -> RigidTransform:
    """Post-goal lift - lift straight up in world frame after releasing brick."""
    # Lift in world frame (not gripper frame) to go straight up
    current_pos = X_WG.translation()
    lifted_pos = current_pos + np.array([0.0, 0.0, 0.2])  # Lift 20cm in world Z
    return RigidTransform(X_WG.rotation(), lifted_pos)
...
if __name__ == "__main__":
    ...
    brick_size = [0.10, 0.08, 0.04]  # Brick dimensions (100mm x 80mm x 40mm) - fits in gripper
